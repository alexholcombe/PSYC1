...
Create plot for PSYC2016 mini-textbook, with just facing-right condition.
...
papaja::apa_beeplot(data= filter(vert, cond=="facing right" | cond=="facing up"),
                    ...
                    , args_error_bars = list(lwd=2.2,length = 0.13))
abline(h=0)
#ggsave("figures/topBiasTwoLtrsFacingRight.png")    ggsave doesn't work for these apa_beeplots! so change to ggplot
...
papaja::apa_beeplot(data= filter(vert, cond=="facing up"),
                    ...
                    , args_error_bars = list(lwd=2.2,length = 0.13))
abline(h=0)
#ggsave("figures/topBiasTwoLtrsFacingUp.png")   ggsave doesn't work for these apa_beeplots!
```
...
Slopes are similar but very shallow for different conditions, see plot but haven't gtotten the equations to render separately so see next snipper
```{r correlate}
...
tbTwoSumm <- tbTwoSumm %>% mutate(cond = case_when(cond == "letter_horiz_0_FALSE" ~ "letters left/right",
                                        cond == "letter_vert_90_FALSE" ~ "letters top/bottom facing down", 
                                        cond == "word_horiz_0_FALSE" ~ "words left/right",
                                          TRUE ~ "unknown"   )) #e
tbTwoSumm$cond <- as.factor(tbTwoSumm$cond) #turn it into numeric to provide a y coordinate for the regression label
...
hh = hh + geom_text(aes(x = .15, y = .67 - .05*as.numeric(cond), label = lm_eqn(lm(leftTopBias ~ authorsPct, tbTwoSumm))), parse = TRUE)
...
The constants in botht the abov and the below from Stack Overflow always print inside a c() and I don't know why.
```{r}
mm<- lm(leftTopBias ~ authorsPct, tbTwoSumm)

cor.test(tbTwoSumm$authorsPct, tbTwoSumm$leftTopBias)
```
...
library(devtools)
source_gist("524eade46135f6348140", filename = "ggplot_smooth_func.R") #https://stackoverflow.com/questions/38345894/r-source-gist-not-working

#https://stackoverflow.com/a/27959418/302378
hh<- ggplot(tbTwoSumm, aes(y=leftTopBias, x=authorsPct, color=cond)) + geom_point() +
      xlab('Author recognition score') + ylab("Left or top bias") + labs(color="Condition") +
...
hh + theme_bw() + ggtitle('normally oriented conditions combined')
```

Also try including letters rotated/flipped to face the other direction, but flip the sign of the bias so that line should still slope upward if author score increases your bias in the cue-specified direction.
